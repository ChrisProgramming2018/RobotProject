...
def train_agent(args, param):
    ...
    pathname = str(args.locexp) + "/" + str(args.env_name) + '-agent-' + str(args.policy)
    pathname += "_batch_size_" + str(args.batch_size) + "_lr_act_" + str(args.lr_actor) 
    pathname += "_lr_critc_" + str(args.lr_critic)
    pathname += "_seed_" + str(args.seed)
    pathname += "_actor_cross_double_update__scale_reward_" + str(args.reward_scalling)
    pathname += "_update_critic_" + str(args.repeat_opt) + "_update_actor_" + str(args.policy_freq)
    pathname += "_actor_cross_start_each_crtitc_own_aug" + str(args.tau)
    text = "TCQ"
    ...
    if args.policy == "TCQ":
        policy = TCQ(state_dim, action_dim, max_action, args)
    ...
    replay_buffer = ReplayBuffer(obs_shape, action_shape, int(args.buffer_size), args.image_pad, args.device, args)
    save_env_vid = False
    total_timesteps = 0
