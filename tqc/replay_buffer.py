...
class ReplayBuffer(object):
    ...
    def __init__(self, obs_shape, action_shape, capacity, image_pad, device, last=10.000):
    ...
    def sample_actor(self, batch_size):
        if self.idx < self.last: 
            bottom  = self.capacity - (self.last - self.idx)
        else:
    ...
    def sample(self, batch_size, actor=False):
        if actor:
            if self.idx < self.last:
                bottom  = self.capacity - (self.last - self.idx)
            else:
